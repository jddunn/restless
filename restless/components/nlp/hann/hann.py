...
class HierarchicalAttentionNetwork():
    ...
    def preprocess_data(self, data_train:object):
        for idx in range(data_train.review.shape[0]):
            ...
            text = text_normalizer.normalize_text(data_train.review[idx],
                          lowercase=True,
                          strip_punct=True,
        ...
        self.data = np.zeros((len(self.texts), MAX_SENTENCE_COUNT, MAX_SENTENCE_LENGTH), dtype='int32')
        ...
        for i, sentences in enumerate(self.reviews):
            for j, sent in enumerate(sentences):
                ...
                for _, word in enumerate(wordTokens):
                    if k < MAX_SENTENCE_LENGTH and self.tokenizer.word_index[word] < VOCABULARY_SIZE:
    ...
    def make_embeddings_matrix(self, embeddings_index):
        embeddings_matrix = np.random.random((len(self.word_index) + 1, self.EMBEDDING_DIM))
    ...
    def build_network_and_train_model(self, embeddings_matrix, model_filepath:str=None):
        ...
        review_input = Input(shape=(MAX_SENTENCE_COUNT, MAX_SENTENCE_LENGTH), dtype='int32')
